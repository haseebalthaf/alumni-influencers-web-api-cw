...
const getBidStatus = async (req, res) => {
  try {
    ...
    const activeMonthWins =
      profile && profile.lastWinMonth === currentMonth
        ? profile.monthlyWins
        : 0;
    const extraWin =
      profile && profile.eventParticipationMonth === currentMonth ? 1 : 0;
    const allowedWins = 3 + extraWin;
    const remainingSlots = Math.max(0, allowedWins - activeMonthWins);
    const sponsorshipBudget =
      profile && Array.isArray(profile.sponsorshipOffers)
        ? profile.sponsorshipOffers.reduce(
    ...
    res.status(500).json({
      message: "Server error while retrieving bid status",
...
const placeBid = async (req, res) => {
  try {
    ...
    const activeMonthWins =
      profile.lastWinMonth === currentMonth ? profile.monthlyWins : 0;
    const extraWin = profile.eventParticipationMonth === currentMonth ? 1 : 0;
    const allowedWins = 3 + extraWin;

    if (activeMonthWins >= allowedWins) {
      return res.status(400).json({
        message: `You have reached your monthly bid limit (${allowedWins} wins) for this month`,
      });
    }
    ...
    if (sponsorshipBudget > 0 && amount > sponsorshipBudget) {
      return res.status(400).json({
        message: `Bid amount cannot exceed available sponsorship budget (£${sponsorshipBudget})`,
      });
    }
    ...
    if (bid) {
      if (amount <= bid.amount) {
        return res.status(400).json({
          message: `New bid amount (${amount}) must be higher than current bid (${bid.amount})`,
        });
      }
      bid.amount = amount;
      bid.date = new Date();
...
const getTodayWinner = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Server error while retrieving today's winner",
      error: process.env.NODE_ENV === "development" ? error.message : undefined,
    });
  }
};
...
const getTomorrowSlot = async (req, res) => {
  try {
    ...
    if (!winner) {
      return res
        .status(404)
        .json({ message: "No slot selected for tomorrow yet" });
    }
    ...
    res.status(500).json({
      message: "Server error while retrieving tomorrow slot",
      error: process.env.NODE_ENV === "development" ? error.message : undefined,
    });
  }
};
...
const getBidHistory = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Server error while retrieving bid history",
      error: process.env.NODE_ENV === "development" ? error.message : undefined,
    });
  }
};
...
const cancelBid = async (req, res) => {
  try {
    ...
    if (!bid) {
      return res
        .status(404)
        .json({ message: "No active bid found for current month" });
    }
