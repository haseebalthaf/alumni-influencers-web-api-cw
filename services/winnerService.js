...
const getAllowedWinsForMonth = (profile, month) => {
  const activeMonthWins =
    profile.lastWinMonth === month ? profile.monthlyWins : 0;
...
const findWinnerForDate = async (targetDate) =>
  Winner.findOne({
    date: {
      $gte: targetDate,
...
const selectWinnerForDate = async ({
  forToday = false,
  strict = true,
} = {}) => {
  ...
  if (!profile) {
    if (strict) {
      throw createHttpError(
        404,
        `Profile not found for highest bidder (${highestBid.user.email})`,
      );
    }
  ...
  const { activeMonthWins, allowedWins } = getAllowedWinsForMonth(
    profile,
    winnerMonth,
  );
